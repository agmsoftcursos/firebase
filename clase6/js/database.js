...
function guardarPerfil(uid, userData) {
  ...
  var refPerfil = database.ref('clase6/users').child(uid);
...
function almacenarAvatar(url, uid) {
  var refPerfil = database.ref('clase6/users').child(uid).child('photoURL');
  refPerfil.set(url);
}
...
function actualizaReferenciaAvatar(nuevaRef, uid) {
  var refAvatar = database.ref('clase6/users').child(uid).child('avatarRef');
