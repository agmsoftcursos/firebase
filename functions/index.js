...
.ref('/clase8/messages/{messageId}').onWrite(event => {
...
exports.addMessage = functions.https.onRequest((req, res) => {
    ...
    admin.database().ref('/clase8/messages').push({original: original}).then(snapshot => {
      // Redirect with 303 SEE OTHER to the URL of the pushed object in the Firebase console.
      //res.redirect(303, snapshot.ref);
      //console.log("SS:", snapshot);
      res.status(200).send("OK: " + snapshot.ref + "/original = " + original);
    });
  });

// Listens for new messages added to /messages/:pushId/original and creates an
// uppercase version of the message to /messages/:pushId/uppercase
exports.makeUppercase = functions.database.ref('/clase8/messages/{pushId}/original')
    .onWrite(event => {
